...
describe('Configuration tests', () => {
  it('Configuration should parse a correct JSON when valid configuration file', async () => {
  ...
  it('Configuration should parse a correct JSON when valid configuration file with same assets name', async () => {
  ...
  it('Configuration should throw a SyntaxError when invalid configuration file', async () => {
    const {expect} = await import('chai');
    expect(() => {
      new Configuration(CONFIG_KO_JSON);
    }).to.throw(SyntaxError);
  });
  it('Configuration should throw a ConfigurationError when bad content configuration file path', async () => {
    const {expect} = await import('chai');
    expect(() => {
      new Configuration(CONFIG_KO_CONTENT);
    }).to.throw(ConfigurationError);
  });
  it('Configuration should throw a ConfigurationError when configuration file with same package name', async () => {
    const {expect} = await import('chai');
    expect(() => {
      new Configuration(CONFIG_KO_DUPLICATE);
    }).to.throw(ConfigurationError);
  });
  it('Configuration should throw a ConfigurationError when incomplete configuration file', async () => {
    const {expect} = await import('chai');
    expect(() => {
      new Configuration(CONFIG_KO_INCOMPLETE);
    }).to.throw(ConfigurationError);
  });
  it('Configuration should throw a ConfigurationError when non existing configuration file path', async () => {
    const {expect} = await import('chai');
    expect(() => {
      new Configuration(NON_EXISTING_FILE);
    }).to.throw(ConfigurationError);
  });
  it('save should save configuration', async () => {
    setChaiAsPromised();
    ...
    const {expect} = await import('chai');
    return fs.rm(savedConfigurationPath, {force: true}).then(async () => {
        ...
        await fs
          ...
          .then(async () => {
            await fs
              ...
              .then(async savedContent => {
                await fs.readFile(CONFIG_OK, 'utf8').then(originContent => {
