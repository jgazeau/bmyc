...
describe('Bmyc tests', () => {
  ...
  it('Bmyc should throw a ConfigurationError when no default configuration file', async () => {
    setChaiAsPromised();
    mockArgs([]);
    const {expect} = await import('chai');
    await expect(Bmyc.main()).to.eventually.be.rejectedWith(ConfigurationError);
  });
  it('Bmyc should create assets and update configuration file when default configuration file', async () => {
    setChaiAsPromised();
    mockArgs([]);
    const {expect} = await import('chai');
    return fs
      ...
      .then(async () => {
        ...
        await Bmyc.main().then(async (updatedConfig: Configuration) => {
          await Promise.all(
            updatedConfig._assets.map(async (asset: Asset) => {
              await fs
            ...
            await fs
              ...
              .then(async updatedConfigContent => {
                await fs
  ...
  it('Bmyc should create assets and update configuration file when specific configuration file', async () => {
    setChaiAsPromised();
  ...
  it(`Bmyc should create assets and update configuration file when up-to-date default configuration file and ${FORCE_OPTION} option`, async () => {
    setChaiAsPromised();
    ...
    return fs
      ...
      .then(async () => {
        ...
        await Promise.all(
          configuration._assets.map(async (asset: Asset) => {
            await asset.setToLatestVersion().then(async () => {
              await fs
                .ensureFile(asset._localPath.toString())
                .then(async () => {
                  await fs
          ...
          .then(async () => {
            await configuration.save().then(async () => {
              await fs
                .readFile(COPY_DEFAULT_CONFIG_FILE, 'utf8')
                .then(configContent => {
  ...
  it(`Bmyc should create assets and update configuration file when up-to-date specific configuration file and ${FORCE_OPTION} option`, async () => {
    setChaiAsPromised();
    mockArgs([
      `--${CONFIG_OPTION}`,
  ...
  it('Bmyc should do nothing when up-to-date default configuration file', async () => {
    setChaiAsPromised();
    mockArgs([]);
    const {expect} = await import('chai');
    return fs
  ...
  it('Bmyc should create 1 asset and update configuration file when incorrect configuration file', async () => {
    setChaiAsPromised();
    ...
    return fs
      ...
      .then(async () => {
        await Bmyc.main().then(async (updatedConfig: Configuration) => {
          await Promise.all(
            updatedConfig._assets.map(async (asset: Asset, index: number) => {
              await fs
                .pathExists(asset._localPath.toString())
                .then(isFileExists => {
                  if (index === 0) {
